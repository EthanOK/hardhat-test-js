...
contract NftExchangeV2UpgradeableT is
    ExchangeDomainV1,
    Initializable,
    PausableUpgradeable,
    OwnableUpgradeable,
    ReentrancyGuardUpgradeable
{
    ...
    function exchangeMul(
        ...
        for (uint256 i = 0; i < len; ++i) {
            ...
            _transferNftToBuyerMul(orders[i], buyer, amounts[i]);

            _transferBuyTokenToSellerMul(
                orders[i],
                buyer,
                payPrice,
                royaltyFees[i]
    ...
    function _transferNftToBuyerMul(
        Order calldata order,
        address toAccount,
        uint256 amount
    ) internal {
        AssetType assertType = order.key.sellAsset.assetType;
        address nftAddress = order.key.sellAsset.token;
        address fromAccount = order.key.owner;
    ...
    function _transferBuyTokenToSellerMul(
        Order calldata order,
        address fromAccount,
        uint256 payAmount,
        uint256 royaltyFee
    ) internal {
        AssetType assertType = order.key.buyAsset.assetType;
        address erc20Address = order.key.buyAsset.token;
        address toAccount = order.key.owner;
